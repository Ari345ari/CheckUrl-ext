// Simple popup implementation for the extension
document.addEventListener('DOMContentLoaded', async function() {
    ...
    try {
        ...
        const currentUrl = tabs[0]?.url || 'No URL available';// Fixed popup implementation for the extension
document.addEventListener('DOMContentLoaded', async function() {
...
console.log('Popup script loaded');        
        // Get scan history and statistics
        const historyResult = await browser.storage.local.get(['scanHistory']);
        const statsResult = await browser.storage.local.get(['statistics']);
        const scanHistory = historyResult.scanHistory || [];
        const stats = statsResult.statistics || { safeSites: 0, maliciousSites: 0, suspiciousSites: 0 };
        
        // Analyze current URL
        let currentScan = { status: 'safe' };
        try {
            currentScan = await browser.runtime.sendMessage({
                url: currentUrl
            });
        } catch (error) {
            console.error('Error analyzing current URL:', error);
            ...
            <div class="extension-popup">
                <div class="header">
                    ...
                    <div class="stats">
                        <span class="stat safe">Safe: ${stats.safeSites || 0}</span>
                        <span class="stat blocked">Blocked: ${stats.maliciousSites || 0}</span>
                        <span class="stat warnings">Warnings: ${stats.suspiciousSites || 0}</span>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    try {
        ...
        console.log('History count:', scanHistory.length);
        
        // Analyze current URL - FIXED: Added action property
        let currentScan = { status: 'safe', threats: [] };
        try {
            currentScan = await browser.runtime.sendMessage({
                action: 'analyzeUrl',  // THIS WAS MISSING!
                url: currentUrl
            });
            console.log('Scan result:', currentScan);
        } catch (error) {
            console.error('Error analyzing current URL:', error);
        }
            ...
            <div class="extension-popup">
                ...
                <div class="current-page">
                    ...
                    <div class="url-status ${currentScan.status}">
                        <div class="status-icon">${getStatusIcon(currentScan.status)}</div>
                        <div class="url-info">
                            <div class="url" title="${currentUrl}">${truncateUrl(currentUrl, 35)}</div>
                            <div class="scan-time">Scanned just now</div>
                        </div>
                        <div class="status-badge">${getStatusBadge(currentScan.status)}</div>
                    </div>
                        ...
                        <div style="display: flex; align-items: start; gap: 8px;">
                            ...
                            <div>
                                <p style="margin: 0 0 8px 0; font-size: 14px; color: #991b1b;">
                                    This page contains malicious content. We recommend leaving immediately.
                                </p>
                                <div style="display: flex; flex-wrap: wrap; gap: 4px;">
                                    ${currentScan.threats.map(threat => `
                                        <span style="background: #dc2626; color: white; padding: 2px 6px; border-radius: 10px; font-size: 11px;">
                ...
                <div style="padding: 16px; padding-top: 0;">
                    ...
                    <div style="max-height: 200px; overflow-y: auto;">
                        ${scanHistory.slice(0, 8).map(scan => `
                            <div style="display: flex; align-items: center; gap: 12px; padding: 8px; border-radius: 6px; margin-bottom: 4px;">
                                <div class="status-icon">${getStatusIcon(scan.status)}</div>
                                <div style="flex: 1; min-width: 0;">
                                    <div class="url" title="${scan.url}">${truncateUrl(scan.url, 30)}</div>
                                    <div style="font-size: 12px; color: #6b7280;">${formatTime(scan.timestamp)}</div>
                                </div>
                                <div class="status-badge">${getStatusBadge(scan.status)}</div>
                            </div>
                        `).join('')}
                        ${scanHistory.length === 0 ? '<p style="text-align: center; color: #6b7280; font-size: 14px; margin: 20px 0;">No scans yet</p>' : ''}
                    </div>
                </div>
        ...
        document.getElementById('scanBtn').addEventListener('click', async () => {
            ...
            try {
                // Send message to content script to rescan
                await browser.tabs.sendMessage(tabs[0].id, { action: 'scanCurrentPage' });
        ...
        root.innerHTML = `
            <div style="padding: 20px; text-align: center;">
                <h3>Check URL Scanner</h3>
                <p style="color: #dc2626;">Error loading extension data</p>
                <p style="font-size: 12px; color: #666;">${error.message}</p>
                <button onclick="window.location.reload()" style="
                    margin-top: 12px; padding: 8px 16px; background: #3b82f6; color: white;
    ...
    return date.toLocaleDateString();
        ...
        root.innerHTML = `
            <div style="padding: 20px; text-align: center;">
                ...
                <p style="font-size: 12px; color: #666;">Please refresh the page and try again</p>
            </div>
        `;
    }
});
...
function formatTime(timestamp) {
    ...
    if (diff < 60) return 'Just now';
    if (diff < 3600) return `${Math.floor(diff / 60)}m ago`;
    if (dif// Fixed popup implementation for the extension
document.addEventListener('DOMContentLoaded', async function() {
    const root = document.getElementById('root');
    ...
    try {
        ...
        root.innerHTML = `
            <div class="extension-popup">
                ...
                <div class="current-page">
                    ...
                    ${currentScan.status === 'malicious' && currentScan.threats ? `
                    <div style="margin-top: 12px; padding: 12px; background: #fef2f2; border: 1px solid #fecaca; border-radius: 8px;">
                        <div style="display: flex; align-items: start; gap: 8px;">
                            <span style="font-size: 14px;">⚠️</span>
                            <div>
        ...
        root.innerHTML = `
            <div style="padding: 20px; text-align: center;">
                <div style="font-size: 32px; margin-bottom: 12px;">🛡️</div>
                <h3 style="margin: 0 0 8px 0;">Check URL Scanner</h3>
                <p style="color: #dc2626; margin: 8px 0;">Error loading extension data</p>
